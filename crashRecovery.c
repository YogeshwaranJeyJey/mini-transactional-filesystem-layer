...
long getLastTxnId(){
    FILE* fp = fopen(CHECKPOINT_FILE_PATH, "r");
    if(!fp){
        fp = fopen(CHECKPOINT_FILE_PATH, "w");
        if(!fp){
            printf("Error occurred while creating checkpoint file!\n");
            return -1;
        }
        fprintf(fp, "0\n");
        fclose(fp);
...
int readWalTransaction(FILE *walFp, wal_Txn_t *walTxn)
{
    ...
    if(strncmp(line, "Operation:", 10) != 0)
        return -1;
...
int undo(wal_Txn_t *walTxn){
    ...
    if(strcmp(walTxn->operation, "CREATE") == 0){
        ...
        if(accountExists(accNo)){
            ...
            printf("Undo not needed! skipping Txn->%ld\n", walTxn->txnId);
        return 1;
    }
    ...
    if(strcmp(walTxn->operation, "TRANSFER") == 0){
        if(!(walTxn->hasOldFrom && walTxn->hasOldTo))
            return 0;
...
int redo(wal_Txn_t* walTxn){
    ...
    if(strcmp(walTxn->operation, "CREATE") == 0){
        ...
        writeCreateToWal(&walTxn->newAccDetails, walTxn->txnId, 1);
        return 1;
    }
    ...
    if(strcmp(walTxn->operation, "UPDATE") == 0){
        if(walTxn->hasOld && walTxn->hasNew){
            ...
            writeUpdateToWal(&walTxn->oldAccDetails, &walTxn->newAccDetails, walTxn->txnId, 1);
            return 1;
        }
        return 0;
    }
    ...
    if (strcmp(walTxn->operation, "TRANSFER") == 0) {
        ...
        writeTransferToWal(&walTxn->oldFromAccDetails, 
        &walTxn->oldToAccDetails, &walTxn->newFromAccDetails, 
        &walTxn->newToAccDetails, walTxn->txnId, 1);
        return 1;
    }
    return 0;
}
...
int updateCheckPoint(long lastTxnId){
    FILE* fp = fopen(CHECKPOINT_FILE_PATH, "w");
    if(!fp){
...
void startRecovery(){
    ...
    long lastTxnId = getLastTxnId();
    
    FILE* walFp = fopen(WAL_TXTFILE_PATH, "r");
    if(!walFp){
        walFp = fopen(WAL_TXTFILE_PATH, "w");
        if(!walFp){
    ...
    FILE* newWalFp = fopen(NEW_WAL_TXTFILE_PATH, "a");
    if(!newWalFp){
        printf("Error creating new wal.txt file!\n");
        return;
    }
    fclose(newWalFp);
    ...
    while(1){
        ...
        if(!walTxn.endTxnFlag){
            printf("Detected Partial transaction!\n");
            printf("Skipping partial transaction->%ld \n", walTxn.txnId);
            continue;
        }

        if(walTxn.txnId <= lastTxnId){
            printf("Skipping txn->%ld Already written to data file!\n", walTxn.txnId);
            continue;
        }

        if(!walTxn.commitedFlag){
            printf("Undoing uncommited transaction if any changes made to original file!->%ld\n", walTxn.txnId);
            if (!undo(&walTxn)){
        ...
        printf("Redoing txn->%ld opeation=%s\n", walTxn.txnId, walTxn.operation);
        if(!redo(&walTxn)){
            printf("Failed to Redo txn %ld! Aborting recovery\n", walTxn.txnId);
            break;
        }

        if(!updateCheckPoint(walTxn.txnId)){
    ...
    remove(WAL_TXTFILE_PATH);
    rename(NEW_WAL_TXTFILE_PATH, WAL_TXTFILE_PATH);
    
    printf("WAL recovery complete! Last checkpoint = %ld\n", lastTxnId);
}